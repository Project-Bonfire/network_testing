#include "ni.h"
#include "packets.h"
#include "plasma.h"
#include "uart.h"
#include "test_plasma.h"
#include "ni_test.h"

#define MY_ADDR     0
#define DST_ADDR    1

int main(int argc, char const *argv[]) {

    unsigned flit;
    unsigned flit_type;
    unsigned payload;
    unsigned curr_addr;
    unsigned counter_value_0 = 0;
    unsigned counter_value_1 = 0;
    unsigned counter_value_2 = 0;
    unsigned packet_counter = 1;

    /* Test UART */
    setup_uart(CPU_SPEED, UART_BAUDRATE);
    uart_puts("UART TEST: If you can read this, then UART output works!\n");

    #if (UART_IN_TEST == 1)

    uart_puts("Please press letter 'b' on the UART terminal:\n");
    char uart_in = uart_getch();

    if (uart_in == 'b')
    {
        uart_puts("UART INPUT TEST PASSED!\n\n");
    }

    else
    {
        uart_puts("UART INPUT TEST FAILED!\n");
        uart_puts("Received following letter: {ASCII:HEX}\n");
        uart_putchar(uart_in);
        uart_putchar(':');
        uart_print_hex(uart_in);
        uart_puts("\n\n");
    }

    #endif


    /* Test reading the NI address */
    curr_addr = memory_read(NI_NODE_ADDRESS);

    uart_print_hex(curr_addr);
    uart_putchar('\n');

    uart_puts("Basic counter test\n");
    counter_value_0 = memory_read(COUNTER_REG);
    uart_print_hex(counter_value_0);

    uart_putchar('\n');

    counter_value_0 = memory_read(COUNTER_REG);
    uart_print_hex(counter_value_0);

    #if (GPIO_TEST == 1)

    /* Test GPIO */
    unsigned gpio_in = memory_read(GPIOA_IN);
    memory_write(GPIO0_SET, gpio_in);

    #endif

    uart_puts("\n\nBeginning communication test\n\n");


    ni_write(build_header(DST_ADDR, 3));
    counter_value_0 = memory_read(COUNTER_REG);
    ni_write(0b1111111111111111111111111111);
    counter_value_1 = memory_read(COUNTER_REG);
    ni_write(0);
    counter_value_2 = memory_read(COUNTER_REG);

    uart_puts("\n\nThe flits of the first packet were sent at the following counter values:\n");
    uart_puts("Header: ");
    uart_print_hex(counter_value_0);
    uart_putchar('\n');

    uart_puts("Body: ");
    uart_print_hex(counter_value_1);
    uart_putchar('\n');

    uart_puts("Tail: ");
    uart_print_hex(counter_value_2);
    uart_putchar('\n');

    while (packet_counter <= SEND_PACKET_COUNT)
    {
        if ((ni_read_flags() & NI_READ_MASK) == 0)
        {
            flit = ni_read();
            flit_type = get_flit_type(flit);

            if (flit_type == FLIT_TYPE_HEADER)
            {
                uart_puts("Sending packet number ");
                uart_print_num(packet_counter, 10, 0);
                uart_putchar('\n');
                ni_write(build_header(DST_ADDR, 3));
                packet_counter++;
            }
            else
            {
                payload = get_flit_payload(flit);
                ni_write(payload);
            }
        }
    }

    /* Run CPU test */
    test_plasma_funcitons();

    return 0;
}
